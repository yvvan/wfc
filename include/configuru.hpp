...
# License
        ...
        If you find this library useful, send a tweet to
[@ernerfeldt](https://twitter.com/ernerfeldt) or mail me at
...
# Version history
        ...
        0.2.4: 2016-08-18 - fix compilation error for when
CONFIGURU_VALUE_SEMANTICS=0 0.3.0: 2016-09-15 - Add option to not align values
(object_align_values) 0.3.1: 2016-09-19 - Fix crashes on some compilers/stdlibs
        ...
        0.4.0: 2017-04-17 - Automatic (de)serialization with
serialize/deserialize with https://github.com/cbeck88/visit_struct 0.4.1:
2017-05-21 - Make it compile on VC++
...
/// CONFIGURU_ON_DANGLING(message_str) is called by check_dangling() if there is
/// any unaccessed keys.
...
struct DocInfo {
  ...
  void append_include_info(std::string &ret,
...
public:
  enum Type {
    ...
    BadLookupType, ///< We are the result of a key-lookup in a Object with no
                   ///< hit. We are in effect write-only.
    Null,
    Bool,
    Int,
    Float,
  ...
  */
  ...
  static Config
  object(std::initializer_list<std::pair<std::string, Config>> values);
  ...
  /// Will still remember file/line when assigned an object which has no
  /// file/line
  ...
  template <typename IntT> IntT as_integer() const {
    static_assert(std::is_integral<IntT>::value, "Not an integer.");
    assert_type(Int);
    check(static_cast<int64_t>(static_cast<IntT>(_u.i)) == _u.i,
  ...
  /// Returns the value or `default_value` if this is the result of a bad
  /// lookup.
  ...
  /// Length of an array
  size_t array_size() const { return as_array().size(); }

  /// Only use this for iterating over an array: `for (Config& e :
  /// cfg.as_array()) { ... }`
  ...
  void push_back(Config value) { as_array().push_back(std::move(value)); }
  ...
  /// Only use this for iterating over an object:
  /// `for (const auto& p : cfg.as_object()) { cout << p.key() << ": " <<
  /// p.value(); }`
  ...
  /// Look up a value in an Object. Returns a BadLookupType Config if the key
  /// does not exist.
  const Config &operator[](const std::string &key) const;

  /// Prefer `obj.insert_or_assign(key, value);` to `obj[key] = value;` when
  /// inserting and performance is important!
  Config &operator[](const std::string &key);

  /// For indexing with string literals:
  template <std::size_t N> Config &operator[](const char (&key)[N]) {
  ...
  /// Returns true iff the value was inserted, false if they key was already
  /// there.
  ...
  /// Look for the given key in this object, and return default_value on
  /// failure.
  template <typename T>
  ...
  /// obj.get_or({"a", "b". "c"}, 42) - like obj["a"]["b"]["c"], but returns 42
  /// if any of the keys are *missing*.
  template <typename T>
  T get_or(std::initializer_list<std::string> keys,
           const T &default_value) const;

  /// obj.get_or({"a", "b". "c"}, 42) - like obj["a"]["b"]["c"], but returns 42
  /// if any of the keys are *missing*.
  std::string get_or(std::initializer_list<std::string> keys,
...
#if !CONFIGURU_VALUE_SEMANTICS // No need for a deep_clone method when all
                               // copies are deep clones.
  ...
  void visit_dangling(
      const std::function<void(const std::string &key, const Config &value)>
          &visitor) const;

  /// Will check for dangling (unaccessed) object keys recursively and call
  /// CONFIGURU_ON_DANGLING on all found.
  ...
  bool has_comments() const { return _comments && !_comments->empty(); }
...
private:
  ...
  Index _line =
      BAD_INDEX; // Where in the source, or BAD_INDEX. Lines are 1-indexed.
...
struct Config::ConfigObject {
  ...
  public:
    ...
    explicit const_iterator(ConfigObjectImpl::const_iterator it)
        : _it(std::move(it)) {}
...
template <> inline signed char Config::get() const {
  return as_integer<signed char>();
}
template <> inline unsigned char Config::get() const {
  return as_integer<unsigned char>();
}
template <> inline signed short Config::get() const {
  return as_integer<signed short>();
}
template <> inline unsigned short Config::get() const {
  return as_integer<unsigned short>();
}
template <> inline signed int Config::get() const {
  return as_integer<signed int>();
}
template <> inline unsigned int Config::get() const {
  return as_integer<unsigned int>();
}
template <> inline signed long Config::get() const {
  return as_integer<signed long>();
}
template <> inline unsigned long Config::get() const {
  return as_integer<unsigned long>();
}
template <> inline signed long long Config::get() const {
  return as_integer<signed long long>();
}
template <> inline unsigned long long Config::get() const {
  return as_integer<unsigned long long>();
}
template <> inline float Config::get() const { return as_float(); }
template <> inline double Config::get() const { return as_double(); }
template <> inline const std::string &Config::get() const {
  return as_string();
}
template <> inline std::string Config::get() const { return as_string(); }
template <> inline const Config::ConfigArrayImpl &Config::get() const {
  return as_array();
}
// template<> inline std::vector<std::string>     Config::get() const { return
// as_vector<T>();   }
...
template <typename T>
T Config::get_or(std::initializer_list<std::string> keys,
                 const T &default_value) const {
...
/// Prints in JSON but in a fail-safe manner, allowing uninitialized keys and
/// inf/nan.
...
inline void clear_doc(
    Config &root) // TODO: shouldn't be needed. Replace with some info of
                  // whether a Config is the root of the document it is in.
{
...
public:
  ParseError(const DocInfo_SP &doc, Index line, Index column,
             const std::string &msg)
      : _line(line), _column(column) {
    _what = doc->filename + ":" + std::to_string(line) + ":" +
...
struct FormatOptions {
  ...
  bool end_with_newline =
      true; ///< End each file with a newline (unless compact).
  ...
  bool distinct_floats =
      true; ///< Print 9.0 as "9.0", not just "9". A must for round-tripping.
  ...
  bool object_align_values =
      true; ///< Add spaces after keys to align subsequent values.
  ...
  /// When printing, write uninitialized values as UNINITIALIZED. Useful for
  /// debugging.
...
inline FormatOptions make_json_options() {
  ...
  options.object_duplicate_keys =
      false; // To be 100% JSON compatile, this should be true, but it is error
             // prone.
...
struct ParseInfo {
  std::map<std::string, Config>
      parsed_files; // Two #include gives same Config tree.
};

/// The parser may throw ParseError.
/// `str` should be a zero-ended Utf-8 encoded string of characters.
/// The `name` should be something akin to a filename. It is only for error
/// reporting.
Config parse_string(const char *str, const FormatOptions &options,
                    const char *name);
Config parse_file(const std::string &path, const FormatOptions &options);

/// Advanced usage:
Config parse_string(const char *str, const FormatOptions &options, DocInfo _doc,
                    ParseInfo &info);
Config parse_file(const std::string &path, const FormatOptions &options,
                  DocInfo_SP doc, ParseInfo &info);

// ----------------------------------------------------------
/// Writes the config as a string in the given format.
/// May call CONFIGURU_ONERROR if the given config is invalid. This can happen
/// if a Config is unitialized (and options write_uninitialized is not set) or
...
/// Writes the config to a file. Like dump_string, but can may also call
/// CONFIGURU_ONERROR if it fails to write to the given path.
void dump_file(const std::string &path, const Config &config,
               const FormatOptions &options);

// ----------------------------------------------------------
// Automatic (de)serialize of most things.
// Include <visit_struct/visit_struct.hpp> (from
// https://github.com/cbeck88/visit_struct) before including <configuru.hpp> to
// get this feature.

#ifdef VISITABLE_STRUCT
template <typename Container> struct is_container : std::false_type {};

// template <typename... Ts> struct is_container<std::array<Ts...> > :
// std::true_type { }; template <typename... Ts> struct
// is_container<std::list<Ts...> > : std::true_type { };
...
template <typename T>
typename std::enable_if<visit_struct::traits::is_visitable<T>::value,
                        Config>::type
...
serialize(const T &some_struct) {
  ...
  visit_struct::apply_visitor(
      [&config](const std::string &name, const auto &value) {
...
void deserialize(std::string *some_string, const Config &config,
                 const ConversionError &on_error);

template <typename T>
typename std::enable_if<std::is_arithmetic<T>::value>::type
deserialize(T *some_value, const Config &config,
            const ConversionError &on_error);

template <typename T, size_t N>
typename std::enable_if<std::is_arithmetic<T>::value>::type
deserialize(T (*some_array)[N], const Config &config,
            const ConversionError &on_error);

template <typename T>
typename std::enable_if<is_container<T>::value>::type
deserialize(T *some_container, const Config &config,
            const ConversionError &on_error);

template <typename T>
typename std::enable_if<visit_struct::traits::is_visitable<T>::value>::type
deserialize(T *some_struct, const Config &config,
            const ConversionError &on_error);

// ----------------------------------------------------------------------------

inline void deserialize(std::string *some_string, const Config &config,
                        const ConversionError &on_error) {
...
deserialize(T (*some_array)[N], const Config &config,
  ...
  if (config.array_size() != N) {
    if (on_error) {
      on_error(config.where() + "Expected array to be " + std::to_string(N) +
               " long.");
    }
  } else {
    for (size_t i = 0; i < N; ++i) {
...
deserialize(T *some_container, const Config &config,
  ...
  if (!config.is_array()) {
    if (on_error) {
      on_error(config.where() +
               "Failed to deserialize container: config is not an array.");
    }
  } else {
...
// ----------------------------------------------------------------------------
// 88 8b    d8 88""Yb 88     888888 8b    d8 888888 88b 88 888888    db 888888
// 88  dP"Yb  88b 88 88 88b  d88 88__dP 88     88__   88b  d88 88__   88Yb88 88
// dPYb     88   88 dP   Yb 88Yb88 88 88YbdP88 88"""  88  .o 88""   88YbdP88
// 88""   88 Y88   88    dP__Yb    88   88 Yb   dP 88 Y88 88 88 YY 88 88 88ood8
// 888888 88 YY 88 888888 88  Y8   88   dP""""Yb   88   88  YbodP  88  Y8
...
*/
    ...
    !defined(CONFIGURU_HAS_BEEN_IMPLEMENTED)
...
// ----------------------------------------------------------------------------
namespace configuru {
void DocInfo::append_include_info(std::string &ret,
                                  const std::string &indent) const {
  if (!includers.empty()) {
    ret += ", included at:\n";
    for (auto &&includer : includers) {
      ret +=
          indent + includer.doc->filename + ":" + std::to_string(includer.line);
...
Config::Config(std::initializer_list<std::pair<std::string, Config>> values)
    : _type(Uninitialized) {
  make_object();
  for (auto &&v : values) {
...
Config::Config(const Config &o) : _type(Uninitialized) { *this = o; }

Config::Config(Config &&o) noexcept : _type(Uninitialized) { this->swap(o); }
...
size_t Config::object_size() const { return as_object()._impl.size(); }
...
bool Config::emplace(std::string key, Config value) {
  ...
  return object
      .emplace(std::move(key),
               Config::ObjectEntry{std::move(value), (unsigned)object.size()})
      .second;
}
...
std::string Config::where() const { return where_is(_doc, _line); }
...
void Config::assert_type(Type exepected) const {
  if (_type == BadLookupType) {
    ...
    const auto message = where() + "Expected " + type_str(exepected) +
                         ", got " + type_str(_type);
...
struct Parser {
  Parser(const char *str, const FormatOptions &options, DocInfo_SP doc,
         ParseInfo &info);

  bool skip_white(Comments *out_comments, int &out_indentation,
  ...
  void tag(Config &var) { var.tag(_doc, _line_nr, column()); }

  State get_state() const { return {_ptr, _line_nr, _line_start}; }
  ...
  Index column() const { return static_cast<unsigned>(_ptr - _line_start + 1); }

  const char *start_of_line() const { return _line_start; }
  ...
  void throw_indentation_error(int found_tabs, int expected_tabs) {
    if (_options.enforce_indentation) {
      ...
      snprintf(buff, sizeof(buff),
               "Bad indentation: expected %d tabs, found %d", found_tabs,
               expected_tabs);
      throw_error(buff);
    }
  }
...
Parser::Parser(const char *str, const FormatOptions &options, DocInfo_SP doc,
               ParseInfo &info)
    : _doc(doc), _info(info) {
  _options = options;
  _line_nr = 1;
...
bool Parser::skip_white(Comments *out_comments, int &out_indentation,
                        bool break_on_newline) {
  auto start_ptr = _ptr;
  out_indentation = 0;
  ...
  while (MAYBE_WHITE[static_cast<uint8_t>(_ptr[0])]) {
    if (_ptr[0] == '\n') {
      ...
      if (_options.enforce_indentation && indentation == "\t") {
        parse_assert(out_indentation != -1,
      ...
      if (found_newline && _options.enforce_indentation) {
        if (indentation == "\t") {
          throw_error("Found a space at beginning of a line. Indentation must "
                      "be done using tabs!");
        } else {
          throw_error("Indentation should be a multiple of " +
                      std::to_string(indentation.size()) + " spaces.");
        }
      }
      ++_ptr;
      out_indentation = -1;
    } else if (_ptr[0] == '/' && _ptr[1] == '/') {
      parse_assert(_options.single_line_comments,
      ...
      // Multi-line comment
      auto state =
          get_state(); // So we can point out the start if there's an error
      _ptr += 2;
      ...
      do {
        if (_ptr[0] == 0) {
          ...
          _ptr += 2;
          parse_assert(_options.nesting_block_comments,
...
Config Parser::top_level() {
  ...
  if (_options.implicit_top_object) {
    ...
    if (IDENT_STARTERS[static_cast<uint8_t>(_ptr[0])] &&
        !is_reserved_identifier(_ptr)) {
      is_object = true;
  ...
  if (is_object) {
    ...
    parse_assert(ret.array_size() <= 1 || _options.implicit_top_array,
...
void Parser::parse_value(Config &dst, bool *out_did_skip_postwhites) {
  ...
  if (_ptr[0] == '"' || _ptr[0] == '@') {
    ...
    parse_assert(_ptr[1] == 'u' && _ptr[2] == 'l' && _ptr[3] == 'l',
                 "Expected 'null'");
    parse_assert(!IDENT_CHARS[static_cast<uint8_t>(_ptr[4])],
                 "Expected 'null'");
    _ptr += 4;
    dst = nullptr;
  } else if (_ptr[0] == 't') {
    parse_assert(_ptr[1] == 'r' && _ptr[2] == 'u' && _ptr[3] == 'e',
                 "Expected 'true'");
    parse_assert(!IDENT_CHARS[static_cast<uint8_t>(_ptr[4])],
                 "Expected 'true'");
    _ptr += 4;
    ...
    parse_assert(_ptr[1] == 'a' && _ptr[2] == 'l' && _ptr[3] == 's' &&
                     _ptr[4] == 'e',
                 "Expected 'false'");
    parse_assert(!IDENT_CHARS[static_cast<uint8_t>(_ptr[5])],
                 "Expected 'false'");
    _ptr += 5;
    dst = false;
  } else if (_ptr[0] == '{') {
    ...
    if (_ptr[0] == '-' && _ptr[1] == 'i' && _ptr[2] == 'n' && _ptr[3] == 'f') {
    ...
    } else if (_ptr[0] == '+' && _ptr[1] == 'i' && _ptr[2] == 'n' &&
               _ptr[3] == 'f') {
      parse_assert(!IDENT_CHARS[static_cast<uint8_t>(_ptr[4])],
                   "Expected +inf");
      parse_assert(_options.inf, "infinity forbidden.");
      _ptr += 4;
      dst = std::numeric_limits<double>::infinity();
    } else if (_ptr[0] == '+' && _ptr[1] == 'N' && _ptr[2] == 'a' &&
               _ptr[3] == 'N') {
      parse_assert(!IDENT_CHARS[static_cast<uint8_t>(_ptr[4])],
                   "Expected +NaN");
...
void Parser::parse_array_contents(Config &array_cfg) {
  ...
  for (;;) {
    ...
    if (IDENT_STARTERS[static_cast<uint8_t>(_ptr[0])] &&
        !is_reserved_identifier(_ptr)) {
      throw_error("Found identifier; expected value. Did you mean to use a "
                  "{object} rather than a [array]?");
    }
...
void Parser::parse_object_contents(Config &object) {
  ...
  for (;;) {
    ...
    if (IDENT_STARTERS[static_cast<uint8_t>(_ptr[0])] &&
        !is_reserved_identifier(_ptr)) {
      parse_assert(_options.identifiers_keys,
      ...
      throw_error("Object key expected (either an identifier or a quoted "
                  "string), got " +
                  quote(_ptr[0]));
    }
    ...
    if (!_options.object_duplicate_keys && object.has_key(key)) {
      ...
      throw_error("Duplicate key: \"" + key + "\". Already set at " +
                  object[key].where());
    }
    ...
    if (_ptr[0] == ':' || (_options.object_separator_equal && _ptr[0] == '=')) {
      parse_assert(_options.allow_space_before_colon || _ptr[0] != ':' ||
                       !space_after_key,
                   "No space allowed before colon");
      _ptr += 1;
      skip_white_ignore_comments();
    } else if (_options.omit_colon_before_object &&
               (_ptr[0] == '{' || _ptr[0] == '#')) {
      // Ok to omit : in this case
    } else {
      if (_options.object_separator_equal &&
          _options.omit_colon_before_object) {
...
void Parser::parse_int(Config &out) {
  ...
  parse_assert(start[0] != '0' || result == 0,
...
void Parser::parse_finite_number(Config &out) {
  ...
  if (_ptr[0] == '0' && _ptr[1] == 'x') {
    parse_assert(_options.hexadecimal_integers,
                 "Hexadecimal numbers forbidden.");
    _ptr += 2;
    auto start = _ptr;
    out = sign *
          static_cast<int64_t>(strtoull(start, const_cast<char **>(&_ptr), 16));
  ...
  if (_ptr[0] == '0' && _ptr[1] == 'b') {
    ...
    auto start = _ptr;
    out = sign *
          static_cast<int64_t>(strtoull(start, const_cast<char **>(&_ptr), 2));
  ...
  const auto MAX_INT_STR =
      (sign == +1 ? "9223372036854775807" : "9223372036854775808");
...
std::string Parser::parse_c_sharp_string() {
  ...
  // ":
  auto state = get_state();
  parse_assert(_options.str_csharp_verbatim,
...
std::string Parser::parse_string() {
  ...
  if (_ptr[1] == '"' && _ptr[2] == '"') {
    ...
    parse_assert(_options.str_python_multiline,
    ...
    for (;;) {
      ...
      if (_ptr[0] == '\t') {
        parse_assert(_options.str_allow_tab,
      ...
      if (_ptr[0] == '\\') {
        ...
        if (_ptr[0] == '"') {
          ...
          if (0xD800 <= codepoint && codepoint <= 0xDBFF) {
            ...
            parse_assert(0xDC00 <= codepoint2 && codepoint2 <= 0xDFFF,
          ...
          parse_assert(_options.str_32bit_unicode,
...
std::string read_text_file(const char *path) {
  ...
  if (fp == nullptr) {
    CONFIGURU_ONERROR(std::string("Failed to open '") + path +
  ...
  auto size = ftell(fp);
  if (size < 0) {
    CONFIGURU_ONERROR(std::string("Failed to find out size of '") + path +
                      "': " + strerror(errno));
  }
  ...
  if (num_read != contents.size()) {
    CONFIGURU_ONERROR(std::string("Failed to read from '") + path +
                      "': " + strerror(errno));
  }
  return contents;
}
...
struct Writer {
  ...
  void write_value(unsigned indent, const Config &config, bool write_prefix,
                   bool write_postfix) {
    ...
    if (config.is_null()) {
      ...
      char temp_buff[64];
      snprintf(temp_buff, sizeof(temp_buff), "%lld",
  ...
  void write_object_contents(unsigned indent, const Config &config) {
    ...
    if (_options.sort_keys) {
      std::sort(begin(pairs), end(pairs),
                [](const ObjIterator &a, const ObjIterator &b) {
    ...
    for (auto &&it : pairs) {
      ...
      if (_compact) {
        _out.push_back(':');
      } else if (_options.omit_colon_before_object && value.is_object() &&
                 value.object_size() != 0) {
        _out.push_back(' ');
      } else {
        _out += ": ";
  ...
  void write_string(const std::string &str) {
    ...
    if (!_options.str_python_multiline || str.find('\n') == std::string::npos ||
        str.length() < LONG_LINE || str.find("\"\"\"") != std::string::npos) {
...
static void write_text_file(const char *path, const std::string &data) {
  ...
  if (num_bytes_written != data.size()) {
    CONFIGURU_ONERROR(std::string("Failed to write to '") + path +
                      "': " + strerror(errno));
  }
}

void dump_file(const std::string &path, const configuru::Config &config,
               const FormatOptions &options) {
