...
// A pattern combined with its hash. This is to make patterns suitable for
// storage in hashtables by allowing their lookup without having to recalculate
// hash values. The class is immutable to ensure that the hash is not
...
// Hashing functor which just forwards the hash of the stored HashedPattern
// member.
...
using PatternPrevalence =
    std::unordered_map<HashedPattern, size_t, PatternHasher>;
...
// Any pattern may be rotated up to 3 times, with each rotation having a
// corresponding reflected version. Sometimes a transformation will not change
// the properties of a pattern, due to symmetry.
//
// Reflection is applied before rotations. Reflections are across the y-axis,
// and rotations are counter-clockwise. Image is first reflected, then rotated.
...
// Instead of storing the full pattern and the full transform information, a
// slightly different form is stored. An index is used to represent the pattern,
// where the index points to an element in a vector. An int is used to represent
// the transform, instead of storing the full transform information. There is a
// one-to-one function between the enumerated transform, and full transform
// information.
...
inline std::ostream &operator<<(std::ostream &stream,
                                const PatternIdentifier &identifier) {
  stream << "{\n";
...
// There are two components. The first is a vector containing the patterns
// themselves. The second is an Array2D which maps each point in the image to
// one of the patterns in this vector. Each element in the Array2D contains an
// index pointing to one of the patterns in the vector, as well as transform
// information required to transform the pattern to the pattern found in the
// image.
...
inline std::ostream &operator<<(std::ostream &stream,
                                const ImagePatternProperties &properties) {
